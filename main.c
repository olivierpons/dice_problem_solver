...
int** permute(const int *nums, int numsSize, int *returnSize, int **block) {
    ...
    *block = (int *)calloc(*returnSize * numsSize, sizeof(int));
...
void add_tuple(
    tuples **tps, int *size, int *capacity, const tuple *a1, const tuple *a2,
...
void add_tuples(
    ...
    int globals[], int len_globals, int local[], int len_local
) {
    ...
    memcpy(t1.local, local, len_local * sizeof(t1.local[0]));
    memcpy(t2.local, local, len_local * sizeof(t2.local[0]));
    ...
    for (int i = 0; i < *size; ++i) {
        if (memcmp((*t)[i].a1.a, t1.a, cut * sizeof(int)) == 0 &&
            memcmp((*t)[i].a2.a, t2.a, (MAX_NORMAL - cut) * sizeof(int)) == 0) {
            return;
        }
        if (memcmp((*t)[i].a2.a, t1.a, cut * sizeof(int)) == 0 &&
            memcmp((*t)[i].a1.a, t2.a, (MAX_NORMAL - cut) * sizeof(int)) == 0) {
            return;
        }
    }
    add_tuple(t, size, capacity, &t1, &t2, globals, len_globals);
}
...
void print_tuple(const tuple *t, const char *delimiter, const char *end) {
    printf("[");
...
void compute_dice_challenge(const char* description, tuple t) {
    int returnSize = 0;
    int *block = NULL;
    int **result = permute(t.a, MAX_NORMAL, &returnSize, &block);
    ...
    process_modifiers(&t, globals, &len_globals, local, &len_local);
