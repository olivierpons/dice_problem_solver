...
#define PRINT_CURRENT_TIME_AND_MORE(more) do { \
     time_t rawtime; \
     struct tm * timeinfo; \
     time(&rawtime); \
...
float getAvailableMemoryPercentage() {
    ...
    char line[256];
    unsigned long long total_memory = 0, available_memory = 0;
    ...
    while (fgets(line, sizeof(line), fp)) {
         ...
         if (strncmp(line, "MemAvailable:", 13) == 0) {
              ...
              break; // No need to continue once we have both values
         }
    }
    fclose(fp);

    if (total_memory == 0) {
         return -1.0f; // To avoid division by zero
    }
    float memory_percentage =  (float)(available_memory * 100.0 / total_memory);
    char memory_str[250];
    snprintf(
        memory_str,
        sizeof(memory_str),
    ...
    PRINT_CURRENT_TIME_AND_MORE(memory_str);
...
#define PRINT_CURRENT_COMB(more) do { \
     PRINT_CURRENT_TIME_AND_MORE(""); \
